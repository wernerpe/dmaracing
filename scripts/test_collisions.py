...
def set_config(env, positions):
    env.states[:] = 0.0
    env.contact_wrenches[:] = 0.0
    env.shove[:] = 0.0
    for ag in range(env.num_agents):
        env.states[0,ag, 0] = positions[ag,0] + 0.1*(np.random.rand()-0.5)
        env.states[0,ag, 1] = positions[ag,1] + 0.1*(np.random.rand()-0.5)
        env.states[0,ag, 2] = positions[ag,2] + 0.1*(np.random.rand()-0.5)
...
def run():
    ...
    conf_idx = 0
    while True:
        set_config(env, positions[conf_idx])
