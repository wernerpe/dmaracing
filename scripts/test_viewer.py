...
state_dummy[0, :, 1] = -1 + 2.0/cfg['sim']['numAgents']*torch.arange(cfg['sim']['numAgents'])
state_dummy[0, :, 2] =np.pi/2 +0*np.pi/cfg['sim']['numAgents']*torch.arange(cfg['sim']['numAgents'])

while True:
    evnt = viewer.render(state_dummy[:,:,0:3])
    print(state_dummy[0,:,0:3])