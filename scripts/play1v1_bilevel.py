...
def play():
    ...
    runner = get_bilevel_ppo_runner(env, cfg_train, None, device = env.device)
    dir_ll, dir_hl, nr_ll, nr_hl = get_run_bilevel(logdir, run_ll=-1, run_hl=-1, chkpt_ll=-1, chkpt_hl=-1)
    mp_ll = "{}/model_{}.pt".format(dir_ll, nr_ll)
    mp_hl = "{}/model_{}.pt".format(dir_hl, nr_hl)

    policy_infos = runner.load(mp_ll, mp_hl)
    policy_ll = runner.get_inference_policy_ll(device = env.device)
    policy_hl = runner.get_inference_policy_hl(device = env.device)
    # if SAVE:
...
if __name__ == "__main__":
    ...
    chkpts = [-1]
    runs = [-1] #['22_09_02_10_40_34_col_0_ar_0.4_rr_0.0', '22_09_02_10_40_34_col_0_ar_0.4_rr_0.0']
    cfg['sim']['numEnv'] = 4
    cfg['sim']['numAgents'] = 2
